...
#define I2S_DIN_GPIO    GPIO_NUM_16     // Data is sent to ESP from ICS-43434 over DOUT
...
// Full-scale for signed 24-bit audio (used for normalization / dBFS)
#define FS_24          8388608.0f   // 2^23

// ---------- CALIBRATION (SERIAL ONLY) ----------
// Set to 1 to run calibration at boot (prints to serial, no MQTT trigger)
#define CAL_ENABLE     1

// Reference SPL (from your SPL meter) at the MIC POSITION during calibration
// Example: set HS5 to 80 dB SPL (meter) at 1 m, then put 80.0f here.
#define CAL_REF_SPL_DB 80.0f

// Duration of calibration capture window
#define CAL_SECONDS    3

// Ignore first part of capture to let tone settle (ms)
#define CAL_SETTLE_MS  300

// Placeholder until calibrated (used if CAL_ENABLE=0)
#define CAL_OFFSET_DB_DEFAULT  94.0f
// ----------------------------------------------
...
// Calibration offset used for SPL estimation: SPL ≈ dBFS + g_cal_offset_db
static float g_cal_offset_db = CAL_OFFSET_DB_DEFAULT;
...
static inline float dbfs_from_rms_norm(float rms_norm)
{
    // rms_norm is RMS normalized to full-scale (0..1)
    if (rms_norm <= 1e-12f) return -120.0f;
    return 20.0f * log10f(rms_norm);
}
...
/* ------------------------------------------------------------
   Calibration (serial-only, no MQTT trigger)
   Computes K where: SPL ≈ dBFS + K
------------------------------------------------------------ */
static void run_calibration_print_only(float ref_spl_db)
{
    ESP_LOGI(TAG, "=== SPL CALIBRATION (SERIAL ONLY) ===");
    ESP_LOGI(TAG, "Play 1 kHz tone and set %.2f dB SPL at mic position (meter).", ref_spl_db);
    ESP_LOGI(TAG, "Capturing %d s (settle %d ms)...", CAL_SECONDS, CAL_SETTLE_MS);

    const int64_t t0_us = esp_timer_get_time();
    const int64_t settle_end_us = t0_us + (int64_t)CAL_SETTLE_MS * 1000LL;
    const int64_t end_us = t0_us + (int64_t)CAL_SECONDS * 1000000LL;
    ...
    while (esp_timer_get_time() < end_us) {
        ...
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "I2S read failed during calibration: %s", esp_err_to_name(err));
            continue;
        }

        if (esp_timer_get_time() < settle_end_us) {
            continue;
        }
        ...
        for (int i = 0; i < n_raw; i++) {
            ...
            float norm = (float)s24 / FS_24;   // [-1, 1)
            sumsq_norm += (double)norm * (double)norm;
            n++;
        }
    }

    if (n == 0) {
        ESP_LOGE(TAG, "Calibration failed: no samples captured.");
        return;
    }

    float rms_norm = sqrtf((float)(sumsq_norm / (double)n));
    float dbfs = dbfs_from_rms_norm(rms_norm);

    // K = SPL_ref - dBFS_measured
    float k_db = ref_spl_db - dbfs;
    g_cal_offset_db = k_db;

    ESP_LOGI(TAG, "=== CAL RESULT ===");
    ESP_LOGI(TAG, "Samples used:      %llu", (unsigned long long)n);
    ESP_LOGI(TAG, "RMS(norm):         %.6f", rms_norm);
    ESP_LOGI(TAG, "Measured level:    %.2f dBFS (RMS)", dbfs);
    ESP_LOGI(TAG, "Offset K:          %.2f dB  (SPL = dBFS + K)", g_cal_offset_db);
    ESP_LOGI(TAG, "Example: -40 dBFS -> %.2f dB SPL", (-40.0f + g_cal_offset_db));
    ESP_LOGI(TAG, "==================");
}
...
static void mic_test_task(void *arg)
{
    ...
    while (esp_timer_get_time() < end_us) {
        ...
        if (now_us >= next_progress_us) {
            ...
            float rms_norm = 0.0f;
            if (rms_count > 0) {
                rms_norm = sqrtf((float)(rms_sum_sq / (double)rms_count));
            }

            float dbfs = dbfs_from_rms_norm(rms_norm);
            float db_spl = dbfs + g_cal_offset_db;
            ...
            ESP_LOGI(TAG, "  SPL:  %.2f dB (K=%.2f)", db_spl, g_cal_offset_db);
...
#if CAL_ENABLE
    run_calibration_print_only(CAL_REF_SPL_DB);
#else
    ESP_LOGW(TAG, "CAL_ENABLE=0. Using default K=%.2f dB", g_cal_offset_db);
#endif
