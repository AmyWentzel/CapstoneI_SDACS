...
static esp_err_t i2c_bus_init(int port, int sda_gpio, int scl_gpio, uint32_t freq_hz)
{
    ...
    if (err == ESP_ERR_INVALID_STATE) {
        // Driver already installed: acceptable for this module.
...
/* ---------- HDC302x CRC + Conversion ---------- */
/*
 * HDC302x uses CRC-8/NRSC-5:
 *  - polynomial 0x31
 *  - init 0xFF
 *  - no final XOR
 */
 ...
 *  Command: 0x24 0x00 (Low Power Mode 0)
 *  Read: 6 bytes => T(MSB),T(LSB),CRC, RH(MSB),RH(LSB),CRC
 *
 * If you ever see intermittent read failures, add a retry loop after the delay.
 */
static esp_err_t hdc302x_read_temp_rh(int port, uint8_t addr, float *temp_c, float *rh_percent)
{
    ...
    const uint8_t cmd[2] = { 0x24, 0x00 }; // Trigger on demand, LP mode 0
    esp_err_t err = i2c_write(port, addr, cmd, sizeof(cmd));
    if (err != ESP_OK) return err;

    // Conversion time margin
    ...
    *rh_percent = 100.0f * ((float)raw_rh / denom);
    ...
    // Optional clamp (keeps logs sane)
...
static void temp_humidity_task(void *arg)
{
    ...
    while (g_ctx.running) {
        ...
        if (err == ESP_OK) {
            ...
            if (g_ctx.topic[0] != '\0') {
                ...
                int len = snprintf(
                    ...
                    "{\"node\":\"%s\",\"t_us\":%" PRIi64 ",\"seq\":%u,\"temp_c\":%.2f,\"rh_percent\":%.2f,\"err\":%u}",
                    g_ctx.node_id,
                ...
                if (len > 0 && len < (int)sizeof(payload)) {
                    ...
                    if (perr != ESP_OK) {
                        g_ctx.publish_fail_count++;
                        if ((g_ctx.publish_fail_count % 10u) == 1u) {
                            ESP_LOGW(TAG, "Temp/humidity publish dropped: %s",
                                     esp_err_to_name(perr));
                        }
                    }
                } else {
                    ESP_LOGW(TAG, "Temp/humidity payload truncated; dropping sample");
                }
            }
        } else {
        ...
        vTaskDelay(pdMS_TO_TICKS(g_ctx.period_ms));
    }
...
void temp_humidity_stop(void)
{
    ...
    g_ctx.running = false;
    for (int i = 0; i < 40 && g_ctx.task != NULL; i++) {
