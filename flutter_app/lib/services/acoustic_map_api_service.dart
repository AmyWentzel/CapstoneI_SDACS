...
class AcousticMapApiService {
  ...
  Future<AcousticMapResult> getMap({String label = 'latest'}) async {
    final uri = Uri.parse('${config.baseUrl}/api/acoustic-map').replace(
      queryParameters: {'label': label},
    );
    final response = await http.get(uri).timeout(const Duration(seconds: 10));
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw Exception('Acoustic map failed: HTTP ${response.statusCode} ${response.body}');
    }
    final decoded = jsonDecode(response.body);
    if (decoded is! Map<String, dynamic>) {
      throw const FormatException('Acoustic-map response was not a JSON object.');
    }
